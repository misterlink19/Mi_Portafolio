<script lang="ts">
  ...
  const baseClasses = 'flex items-center space-x-4 hover:scale-105 transition-transform duration-200';
...
<div
  class="{baseClasses}"
  transition:fly={{ x: -20, duration: 400, delay: delay, easing: quintOut }}
>
  ...
  <div>
    {#if href}
      <a href={href} target="_blank" rel="noopener" class="font-semibold text-gray-900 dark:text-white hover:underline">
        {title}
      </a>
