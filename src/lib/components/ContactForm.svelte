<script lang="ts">
	...
	async function handleSubmit(event: Event) {
		// Añadir 'event: Event' como parámetro
		...
		try {
			...
			if (web3formsResponse.ok) {
				...
				generalError =
					errorData.message || (m['contact.errorMessage']?.() ?? 'Error al enviar el mensaje.');
			}
		} catch (error) {
			...
			generalError =
				m['contact.errorMessage']?.() ?? 'Algo salió mal. Por favor, intenta de nuevo más tarde.';
		} finally {
