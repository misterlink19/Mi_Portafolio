...
<script lang="ts">
    ...
    /**
     * Consola de depuración para mostrar el estado de las variables de ruta y localización.
     * Se activa cada vez que `currentPath`, `currentLocale` o `basePath` cambian.
     */
    $: console.log(
        '%c[Header]',
...
<header class="fixed top-0 w-full bg-white/80 dark:bg-gray-900/80
    ...
    <nav class="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center space-x-8">
            ...
            {#key currentLocale}
                {#each navItems as item}
                    ...
                    >
                      {item.label()} <!-- Llama a la función de mensaje para obtener el texto traducido -->
                    </a>
                {/each}
            {/key}
        </div>
        ...
        <div class="flex items-center space-x-4">
            ...
            >
                <Globe class="w-5 h-5"/> <!-- Icono de globo terráqueo -->
                <span class="text-sm font-medium">
                    {currentLocale.toUpperCase()} <!-- Muestra el código del idioma en mayúsculas -->
                </span>
            </button>
