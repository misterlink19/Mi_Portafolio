<script lang="ts">
    ...
    const getColorClass = (t: 'work' | 'education') =>
        t === 'work' ? 'bg-blue-600 dark:bg-blue-400' : t === 'education' ? 'bg-green-600 dark:bg-green-400' : 'bg-gray-600 dark:bg-gray-400';
</script>
...
<div bind:this={timelineContainer} class="relative">
    ...
    {#each items as item, index}
        <div class="relative flex items-center mb-8" data-item-id={item.id}>
                    ...
                    <div transition:fly={{ y: 30, duration: 600, delay: index * 100, easing: quintOut }}>
                        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-shadow duration-300">
                            <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">
                                {item.type === 'work' ? (item as WorkTimelineItem).position : (item as EducationTimelineItem).degree}
                            </h3>
                            <p class="font-medium mb-1" class:text-blue-600={type === 'work'} class:dark:text-blue-400={type === 'work'} class:text-green-600={type === 'education'} class:dark:text-green-400={type === 'education'}>
                                {item.type === 'work' ? (item as WorkTimelineItem).company : (item as EducationTimelineItem).institution}
                            </p>
                            <p class="text-gray-500 dark:text-gray-400 text-sm mb-2">
                                {item.startYear} - {item.endYear ?? m['timeline.present']()} </p>
                            ...
                            {#if item.type === 'work' && (item as WorkTimelineItem).technologies && (item as WorkTimelineItem).technologies.length > 0}
                                <div class="flex flex-wrap gap-2 mt-3 {index % 2 === 0 ? 'justify-end' : 'justify-start'}">
            ...
            <div class="absolute left-1/2 transform -translate-x-1/2 w-4 h-4 {getColorClass(type)} rounded-full border-4 border-white dark:border-gray-900 z-10"></div>
        </div>
    {/each}
</div>