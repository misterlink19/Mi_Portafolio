...
<script lang="ts">
	...
	const getColorClass = (t: string) =>
		t === 'work' ? 'bg-blue-600' : t === 'education' ? 'bg-green-600' : 'bg-gray-600';
	...
	<div class="absolute left-0 top-0 h-full">
		{#each yearMarkers.filter((_, i) => i % 2 === 0) as year, i}
			<div class="absolute flex items-center" style="top: {i * 120}px">
				<div
					class="w-6 h-6 rounded-full {getColorClass(
						type
		...
		{#each processedItems as item, idx}
			<div data-item-id={item.id ?? `gap-${idx}`} class="relative">
				...
				<div
					...
					style="opacity: {visibleItems.has((item.id ?? `gap-${idx}`).toString())
						? 1
						: 0.3}; transition: opacity 0.5s ease;"
				></div>
				...
				{#if item.isGap}
					...
					{#if visibleItems.has(`gap-${idx}`)}
						<div transition:fly={{ x: 50, duration: 600, easing: quintOut }}>
							<div
								class="bg-gray-100 dark:bg-gray-700 rounded-xl p-4 border-2 border-dashed border-gray-300 dark:border-gray-600 text-center text-gray-500 dark:text-gray-400"
							>
					...
					{:else}
						<div
							class="bg-gray-100 dark:bg-gray-700 rounded-xl p-4 border-2 border-dashed border-gray-300 dark:border-gray-600 opacity-30"
						>
							<p class="text-sm font-medium">
								{m['timeline.gap']({ n: item.duration, s: pluralSuffix(item.duration) })}
							</p>
				...
				{:else}
					...
					{#if visibleItems.has((item.id ?? `gap-${idx}`).toString())}
						<div transition:fly={{ x: 50, duration: 600, easing: quintOut }}>
							<div
								class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-shadow"
							>
