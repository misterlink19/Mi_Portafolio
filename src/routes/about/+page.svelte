...
<div class="container mx-auto p-4 md:p-8 font-inter antialiased text-gray-900 dark:text-white">
            ...
            <div data-animate-id="personal-info-card">
                {#if mounted && visibleElements.has('personal-info-card')}
                    <div transition:fly={{ y: 30, duration: 600, delay: 200, easing: quintOut }}>
                        <div class="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl flex flex-col items-center text-center border border-gray-200 dark:border-gray-700">
                            <!-- Foto de perfil -->
                            <img
                            ...
                            <h1 class="text-4xl lg:text-5xl font-extrabold text-gray-900 dark:text-white mb-3 leading-tight">
                                {personalInfo.name}
                            </h1>
                            ...
                            <div class="flex justify-center space-x-6 mt-auto">
                                <a href={`mailto:${personalInfo.email}`} class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200 text-lg flex items-center transition-colors">
            ...
            <div data-animate-id="technologies-card">
                {#if mounted && visibleElements.has('technologies-card')}
                    <div transition:fly={{ y: 30, duration: 600, delay: 400, easing: quintOut }}>
                        <div class="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl flex flex-col border border-gray-200 dark:border-gray-700">
                            <h3 class="text-3xl font-bold text-gray-800 dark:text-white mb-6 text-center border-b-2 border-blue-500 pb-4">
                                {m['section.technologies']()}
                            </h3>
                            <!-- Grid para mostrar logos y nombres de tecnologías -->
                            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-6 flex-grow">
    ...
    <section class="mb-16">
        <h2 class="text-4xl font-bold text-gray-800 dark:text-white mb-8 text-center" data-animate-id="work-experience-title">
    ...
    <section class="mb-16">
        <h2 class="text-4xl font-bold text-gray-800 dark:text-white mb-8 text-center" data-animate-id="education-title">
                ...
                {#if mounted && visibleElements.has('technical-skills-card')}
                    <div transition:fly={{ y: 30, duration: 600, delay: 100, easing: quintOut }}>
                        <div class="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700">
                            <h3 class="text-2xl font-semibold text-gray-800 dark:text-white mb-5 border-b-2 border-blue-500 pb-2">
                            ...
                            <ul class="list-none text-gray-700 dark:text-gray-300 space-y-4">
                                {#each processedTechnicalSkillsForDisplay as skill, i}
                                    <SkillItem skill={skill} index={i} variant="default" />
                                {/each}
                            </ul>
                        </div>
                    </div>
                            ...
                            <ul class="list-none text-gray-700 dark:text-gray-300 space-y-4">
                                {#each skills.certificates as cert, index}
                                    <li
                                        ...
                                        transition:fly={{ x: -20, duration: 400, delay: index * 50, easing: quintOut }}
                                    >
                                        <h4 class="font-semibold text-gray-900 dark:text-white text-lg">{getMessage(cert.nameKey)}</h4>
                                        <p class="text-gray-600 dark:text-gray-400 text-base">{getMessage(cert.issuerKey)} - {cert.date}</p>
                                    </li>
                                {/each}
                            </ul>
                        </div>
